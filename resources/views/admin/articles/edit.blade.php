...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                <span class="p-1.5 rounded-lg bg-orange-100 text-orange-600 text-sm">📝</span>
                <h2 class="text-base font-black text-slate-900">Edit Artikel & Optimasi SEO / GEO</h2>
            </div>
            <p class="text-xs text-slate-500 mt-1">Perbarui konten artikel, kelola media gambar, dan maksimalkan kata kunci untuk ranking #1 Google serta AI Search Engine.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('articles.show', $article->slug) }}" target="_blank" class="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition flex items-center gap-1.5">
                <span>👁️</span> Pratinjau Publik ↗
            </a>
    ...
    <!-- Main Edit Form -->
    <form id="articleForm" action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6 text-xs">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-5">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3 pt-2">
                        <label class="inline-flex items-center gap-2.5 cursor-pointer font-semibold text-slate-700 p-2 rounded-xl bg-slate-50 border border-slate-200 w-full hover:bg-orange-50/50 transition">
                            <input type="checkbox" name="is_published" value="1" {{ old('is_published', $article->is_published) ? 'checked' : '' }} class="rounded border-slate-300 text-orange-600 focus:ring-0 w-4 h-4">
                            <span class="text-xs">Artikel Aktif / Diterbitkan di Website</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" id="articleTitle" name="title" value="{{ old('title', $article->title) }}" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none focus:border-orange-500 focus:bg-white font-bold transition shadow-sm">
            </div>

            <div>
                <div class="flex justify-between items-center mb-1.5">
                    <label class="font-bold uppercase tracking-wider text-slate-700">Slug URL</label>
                    <span class="text-[10px] text-slate-400 font-normal">URL ramah SEO (hanya huruf kecil dan tanda hubung '-')</span>
                </div>
                <div class="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 focus-within:border-orange-500 focus-within:bg-white transition">
                    ...
                    <input type="text" id="articleSlug" name="slug" value="{{ old('slug', $article->slug) }}" class="w-full bg-transparent py-1 text-xs font-mono text-slate-800 focus:outline-none">
                </div>
            </div>

            <div>
                <label class="block font-bold uppercase tracking-wider text-slate-700 mb-1.5">Ringkasan / Excerpt Singkat</label>
                <textarea name="excerpt" rows="2" class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 focus:outline-none focus:border-orange-500">{{ old('excerpt', $article->excerpt) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mb-3 flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl border border-slate-200">
                        <img src="{{ asset($article->featured_image) }}" alt="{{ $article->featured_image_alt ?? $article->title }}" class="h-16 w-24 object-cover rounded-lg border border-slate-200 shadow-sm">
                        <div class="text-[11px] text-slate-600">
                            <span class="font-bold block text-slate-800">Gambar Saat Ini</span>
                            <span class="text-slate-400 text-[10px]">Unggah file baru di bawah jika ingin mengganti.</span>
                        </div>
                    </div>
                    @endif
                ...
                <div>
                    ...
                    <input type="text" name="featured_image_alt" value="{{ old('featured_image_alt', $article->featured_image_alt ?? $article->title) }}" placeholder="Contoh: Jasa pembuatan website profesional dan cepat di Bogor NazwaGraha" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-orange-500">
                    <p class="text-[11px] text-slate-500 mt-1.5 leading-relaxed">
                        💡 <strong>Tips SEO:</strong> Jelaskan isi gambar secara spesifik dan sisipkan kata kunci utama. Alt text digunakan Google Image Search untuk membaca konten dan menambah poin skor Core Web Vitals.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div id="editorCardContainer" class="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-3 transition-all duration-200">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span id="editorStats" class="text-[11px] text-slate-400 font-mono">0 kata | 0 karakter</span>
                </div>
            </div>
            ...
            <textarea name="content" id="articleContentInput" class="hidden" required>{{ old('content', $article->content) }}</textarea>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="metaTitleInput" name="meta_title" value="{{ old('meta_title', $article->meta_title) }}" placeholder="Judul memikat dengan keyword utama (Rekomendasi 50 - 60 karakter)" class="w-full bg-slate-800/90 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-orange-500">
                    ...
                    <div>
                        ...
                        <input type="text" id="metaKeywordsInput" name="meta_keywords" value="{{ old('meta_keywords', $article->meta_keywords) }}" placeholder="Contoh: jasa pembuatan website bogor, web developer ciawi, harga web murah jabodetabek, seo specialist" class="w-full bg-slate-800/90 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-orange-500 font-mono">
                    ...
                    <div>
                        ...
                        <input type="text" name="geo_target_region" value="{{ old('geo_target_region', $article->geo_target_region ?? 'Bogor, Ciawi, Jabodetabek') }}" placeholder="Contoh: Bogor, Ciawi, Jabodetabek, Jawa Barat, Seluruh Indonesia" class="w-full bg-slate-800/90 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-orange-500">
                    ...
                    <div>
                        ...
                        <textarea name="geo_summary" rows="3" placeholder="Tulis 2-3 kalimat padat berupa fakta & solusi langsung (problem -> solusi NazwaGraha Pratama -> ajakan kontak). Format ini dirancang agar langsung dikutip oleh ChatGPT, Google Gemini, dan Perplexity saat pengguna mencari solusi." class="w-full bg-slate-800/90 border border-slate-700 rounded-xl p-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-orange-500 leading-relaxed">{{ old('geo_summary', $article->geo_summary) }}</textarea>
                    </div>
                    ...
                    <div class="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
                        ...
                        <div id="serpContainer" class="mt-4 p-4 bg-white rounded-xl text-slate-800 shadow-md font-sans transition-all max-w-full">
                            ...
                            <div class="flex items-center gap-2 mb-1.5">
                                ...
                                <div class="leading-tight">
                                    ...
                                    <div id="serpUrl" class="text-[11px] text-slate-500 font-mono truncate">
                                        https://nazwagraha.com/artikel/{{ $article->slug }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <p id="serpSnippet" class="text-xs text-[#4d5156] mt-1.5 leading-relaxed line-clamp-3">
                                {{ $article->meta_description ?? ($article->excerpt ?? 'Deskripsi artikel informatif dan profesional dari NazwaGraha Pratama Solusi IT.') }}
                            </p>
                            ...
                            <div id="serpTags" class="mt-3 pt-2 border-t border-slate-100 flex flex-wrap gap-1">
                                @if($article->meta_keywords)
                                    @foreach(explode(',', $article->meta_keywords) as $kw)
                                        @if(trim($kw))
                                            <span class="text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-medium"># {{ trim($kw) }}</span>
                                        @endif
                                    @endforeach
                                @else
                        ...
                        <div class="mt-4 space-y-2 text-[11px]">
                            ...
                            <div class="flex items-center gap-2 text-slate-300" id="chkTitle">
                                <span class="text-emerald-400">✓</span> Judul artikel menarik & mengandung kata kunci
                            </div>
                            <div class="flex items-center gap-2 text-slate-300" id="chkImageAlt">
                                <span class="text-emerald-400">✓</span> Alt text gambar utama telah dikonfigurasi
                            </div>
                            <div class="flex items-center gap-2 text-slate-300" id="chkHeading">
                                <span class="text-emerald-400">✓</span> Struktur artikel memuat H2/H3 dan paragraf jelas
                            </div>
                            <div class="flex items-center gap-2 text-slate-300" id="chkGeo">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4 pt-2">
            <button type="submit" id="saveArticleBtn" class="w-full sm:flex-1 py-4 rounded-xl orange-gradient text-white font-extrabold text-sm shadow-lg hover:shadow-orange-500/40 transition flex items-center justify-center gap-2 cursor-pointer">
                <span>💾</span> Simpan Perubahan Artikel & Optimasi SEO
            </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSerp() {
        serpTitle.innerText = metaTitleInput.value || articleTitle.value || 'Judul Artikel Belum Diisi';
